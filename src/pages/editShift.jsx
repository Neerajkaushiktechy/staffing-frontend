...
const EditShift = () => {
    ...
    useEffect(() => {
        const fetchCoordinators = async () => {
            if (facility) {
                try {
                    const res = await get(
                        getCoordinatorsByFacility_url(facility),
                        true
                    );
    ...
    useEffect(() => {
        if (coordinator && coordinators.length > 0) {
            const selected = coordinators.find(
                (c) => c.id.toString() === coordinator.toString()
            );
            if (selected) {
    ...
    const getShiftDetailsById = async () => {
        try {
            ...
            if (res.data.status === 200) {
                ...
                if (shift.nurse_id) {
                    const nurseRes = await get(
                        getNurseById_url(shift.nurse_id),
                        true
                    );
                    ...
                    <div>
                        ...
                        >
                            ...
                            {coordinators.map((coordinator) => (
                                ...
                                >
                                    {coordinator.coordinator_first_name}{' '}
                                    {coordinator.coordinator_last_name}
                                </option>
                            ))}
                        </select>
                    </div>
