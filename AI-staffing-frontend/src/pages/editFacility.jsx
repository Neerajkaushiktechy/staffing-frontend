...
import { getFacilityById_url, editFacility_url, getNurseType_url, deleteService_url, deleteCoordinator_url } from "../urls/adminUrls";
...
export const EditFacility = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const transformedCoordinators = coordinators_array.map(coordinator => ({
        id: coordinator.id || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data.status === 200) {
        ...
        toast.error("coordinator phone or email already exists");
  ...
  return (
    <div className="flex flex-col justify-center items-center pt-10 w-full px-4">
      ...
      <button
      onClick={() => handleCoordinatorDelete(coordinator.id, index)}
